...
| `renderer/studyCircle/` | 学习圈答疑置辩、典型案例与项目化学习；包含列表、详情、讨论及材料查看。 |
| `renderer/systemExercise/` | 系统练习范围选择、官方题目页作答、逐题保存与最终提交。 |
| `renderer/schoolExercise/` | 校本练习开通校验、校本教材/章节选择、题目作答与练习结果保存。 |
| `renderer/me/` | “我的”页面。 |
...
| 学习圈 | `renderer/studyCircle/`、`common/study-circle-access.js`、两端 `studyCircle*` API；`learningProjectCfg` 三位分别控制答疑置辩/项目化学习/典型案例，调试模式可绕过；答疑支持提问、回复、公开、结束、删除和附件，案例支持详情及点赞，项目支持详情、小组讨论、组长成果提交和教师总结 |
| 系统练习 | `renderer/systemExercise/` 与两端 `systemExercise*` API；按原版区分章节/知识点选题，答案由官方题目页保存，最后调用 `brushingSubmit` |
| 校本练习 | `renderer/schoolExercise/` 与两端 `schoolExercise*` API；普通模式服从 `isSchoolDoExercise`，调试模式可绕过；最终按题汇总 `questionRef` |
